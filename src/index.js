...
async function initBackEnd() {
  ...
  try {
    ...
    Logger.info(`${LOG_NS} connection with Janus created`); // LOG_NS : 파일 이름
...
function initFrontEnd() {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('leave', async (evtdata = {}) => {
      Logger.info(`${LOG_NS} ${remote} leave received ${evtdata}`);
      const { _id, data: leavedata = {} } = evtdata;
      Logger.info(_id);
      Logger.info(leavedata.feed);

      const handle = clientHandles.getHandleByFeed(leavedata.feed);
      if (!checkSessions(janodeSession, handle, socket, evtdata)) {
